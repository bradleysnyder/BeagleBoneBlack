...
Ov7670::Ov7670() {
	...
	printf("About to initialize a device with addr: %d\n", OV7670_I2C_ADDR);
	camI2CIntf = new I2CIntf(MODEMDEVICE_I2C_CAM,OV7670_I2C_ADDR); //changed from MODEDEVICE_I2C_CAM

	// initialize camera for reading
	//gpio0->setBit(BIT_PWDN, 0);
	//think I have to make sure these line up with how they're defined in the header file
	gpio3->setBit(BIT_OE, 1);
	gpio0->setBit(BIT_WE, 0);

	// reseting..
	//gpio2->setBit(BIT_RST, 0);
	gpio3->setBit(BIT_RRST, 0);
	//gpio2->setBit(BIT_WRST, 0);
	usleep(1000*200);
	//gpio2->setBit(BIT_RST, 1);
	gpio3->setBit(BIT_RRST, 1);
...
void Ov7670::io_setup()
{
	...
	printf("Exported all pin banks and setting pin I/O mode\n");
	...
	gpio1 = new GpioIntf(GPIO1_BASE);
	gpio1->setDirection(BIT_VSYNC, GPIO_DIR_IN);
	...
	gpio3 = new GpioIntf(GPIO3_BASE);
	gpio3->setDirection(BIT_OE, GPIO_DIR_OUT);
	gpio3->setDirection(BIT_RRST, GPIO_DIR_OUT);
